...
#ident "$Id: shadowio.c,v 1.4 2010/03/21 14:56:59 jbj Exp $"
...
{
	const struct spwd *sp = (const struct spwd *) ent;
...
{
	struct spwd *sp = (struct spwd *) ent;
...
{
	return (const struct spwd *) commonio_locate (&shadow_db, name);
}
...
{
	return (const struct spwd *) commonio_next (&shadow_db);
}
