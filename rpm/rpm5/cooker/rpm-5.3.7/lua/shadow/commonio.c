...
#ident "$Id: commonio.c,v 1.3 2010/03/21 14:56:59 jbj Exp $"
 ...
 * target.
 */
int lrename (const char *_old, const char *_new)
{
...
#if defined(S_ISLNK)
	struct stat sb = { 0 };
	if (lstat (_new, &sb) == 0 && S_ISLNK (sb.st_mode)) {
		if (realpath (_new, resolved_path) == NULL) {
...
commonio_sort (struct commonio_db *db, int (*cmp) (const void *, const void *))
{
	...
	entries = (struct commonio_entry **) malloc (n * sizeof (*entries));
	if (entries == NULL)
		return -1;
