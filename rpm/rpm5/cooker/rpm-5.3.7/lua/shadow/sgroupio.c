...
#ident "$Id: sgroupio.c,v 1.4 2010/03/21 14:56:59 jbj Exp $"
...
{
	const struct sgrp *sg = (const struct sgrp *) ent;
...
{
	struct sgrp *sg = (struct sgrp *) ent;
...
{
	const struct sgrp *gr = (const struct sgrp *) ent;
...
{
	return (const struct sgrp *) commonio_locate (&gshadow_db, name);
}
...
{
	return (const struct sgrp *) commonio_next (&gshadow_db);
}
