...
#
# $Id: logio.awk,v 1.4 2009/10/12 19:03:48 jbj Exp $
#
...
/^[	 ]*(DB|ARG|DBT|LOCKS|PGDBT|PGDDBT|POINTER|TIME)/ {
	...
	if ($1 == "DB" || $1 == "ARG" || $1 == "TIME") {
		sizes[nvars] = sprintf("sizeof(uint32_t)");
		if ($3 != "uint32_t")
			is_uint = 1;
	} else if ($1 == "POINTER")
		...
		sizes[nvars] =\
		    sprintf("sizeof(uint32_t) + (%s == NULL ? 0 : %s->size)",\
		    $2, $2);
		is_dbt = 1;
		if ($1 == "PGDBT")
			hdrdbt = vars[nvars];
...
END {
	...
	if (!dbprivate) {
		...
		printf(\
		    ...
		    env_type) >> PFILE
		printf(\
	"\t    int (*)(%s *, DBT *, DB_LSN *, db_recops), uint32_t));\n",\
		    env_type) >> PFILE
	}
	...
	for (i = 0; i < num_funcs; i++) {
		...
		printf("\tif ((ret = __db_add_recovery%s(%s, ",\
		    dbprivate ? "_int" : "_rpmdb", env_var) >> PFILE
		printf("dtabp,\n") >> PFILE
	...
	for (i = 0; i < num_funcs; i++) {
		if (functable[i] == 1)
			continue;
		printf("\tif ((ret = __db_add_recovery_int(env, ",\
		    dbprivate ? "_int" : "_rpmdb", env_var) >> CFILE
		printf("dtabp,\n") >> CFILE
...
function log_function()
{
	...
	printf("\tuint32_t ") >> CFILE
	if (is_dbt)
		printf("zero, ") >> CFILE
	if (is_uint)
		printf("uinttmp, ") >> CFILE
	printf("rectype, txn_num;\n") >> CFILE
	printf("\tunsigned int npad;\n") >> CFILE
	printf("\tuint8_t *bp;\n") >> CFILE
	printf("\tint ") >> CFILE
	if (dbprivate) {
	...
	printf("\tif (npad > 0)\n") >> CFILE
	printf("\t\tmemset((uint8_t *)logrec.data + logrec.size ") >> CFILE
	printf("- npad, 0, npad);\n\n") >> CFILE
	printf("\tbp = logrec.data;\n\n") >> CFILE
	...
	for (i = 0; i < nvars; i++) {
		if (modes[i] == "ARG" || modes[i] == "TIME") {
	       ...
	       "\t\t\t    (PAGE *)bp, (size_t)%s->size, (DBT *)%s, 0)) != 0)\n",
			...
			printf(\
			    "\tuinttmp = (uint32_t)dbp->log_filename->id;\n")\
			    >> CFILE
	...
	if (dbprivate) {
		...
		printf("\t    (uint32_t)(bp - (uint8_t *)") >> CFILE
...
function print_function()
{
	...
	for (i = 0; i < nvars; i ++) {
		...
		if (modes[i] == "DBT" ||
			...
			printf("\t\tch = ((uint8_t *)argp->%s.data)[i];\n",\
			    vars[i]) >> PFILE
